  ...
  return (
    <Wrapper>
      ...
      <Control>
        ...
        <div>
          {exif && <div>{JSON.stringify(exif)}</div>}
          <ImagePreview
            image={image}
            onClick={() => {
              setShowImage(!showImage);
          ...
          onClick={async () => {
            if (camera.current) {
              ...
              const options = {
                ...
                gps: {
                  pick: ["GPSLatitudeRef", "GPSLatitude", 0x0003, 0x0004],
                },
                interop: false,
                ifd1: false, // thumbnail
              };
              // console.log("options", options);
              const trimmedPhoto = (photo as string).substring(23);
              console.log("trimmedPhoto", trimmedPhoto);
              const gps = await exifr.parse(trimmedPhoto, options);
              console.log("gps", gps);
              const _exif = await exifr.parse(trimmedPhoto);
              setExif(_exif || null);
